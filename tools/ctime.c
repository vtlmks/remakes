...
#define TIME_DIFF_EPOCH 11644473600LL
...
static long
  _read(int Handle, void *Data, long Count)
{
    long ReadBytes = 0;
    while (ReadBytes < Count)
    {
        long NextReadBytes = read(Handle, Data + ReadBytes, Count - ReadBytes);
        if (NextReadBytes <= 0)
        {
            ReadBytes = 0;
            break;
...
GetClock(void)
{
    ...
    if (clock_gettime(CLOCK_MONOTONIC, &TimeVal))
    {
        fprintf(stderr, "ERROR: Clock call failed, timing will be inaccurate!\n");
    }

      Result = TimeVal.tv_sec * 1000 + TimeVal.tv_nsec / 1000000LLU;
...
GetDate(void)
{
    ...
    if (gettimeofday(&FileTime, 0))
    {
        fprintf(stderr, "ERROR: Time of day call failed, timing will be inaccurate!\n");
    }
    FileTime.tv_sec = time(0);

     TotalTime = FileTime.tv_sec * 10000000 + FileTime.tv_usec * 10;
    // NOTE(michiel): Add difference in epoch. Linux: 1970-01-01 UTC, Windows: 1601-01-01 UTC
    // TODO(michiel): Correct implementation should account for all kinds of stuff...
    TotalTime += TIME_DIFF_EPOCH * 10000000LL;
    Result.E[0] = TotalTime & 0xFFFFFFFF;
Result.E[1] = TotalTime >> 32;
...
PrintDate(timing_file_date Date)
{
    ...
    TotalTime = ((u64)Date.E[1] << 32) | Date.E[0];
    Time = (TotalTime / 10000000LL) - TIME_DIFF_EPOCH;
    LocalTime = localtime(&Time);
    strftime(StrBuf, sizeof(StrBuf), "%Y-%m-%d %H:%M:%S", LocalTime);
...
{
    u64 A64, B64;
    A64 = ((u64)A.E[1] << 32) | A.E[0];
    B64 = ((u64)B.E[1] << 32) | B.E[0];
...
DayIndex(timing_file_date A)
{
    ...
    TotalTime = (u64)Time * 10000000;

    AD = ((double)TotalTime * (0.0001)) / (1000.0 * 60.0 * 60.0 * 24.0);

    return AD; // LocalTime->tm_yday;
}
...
ReadAllEntries(int Handle)
{
    ...
    if(FileSize > 0)
    {
        ...
        {
            fprintf(stderr, "ERROR: Unable to allocate %ld for storing timing entries.\n", EntriesSize);
        }
    }
    else
    {
